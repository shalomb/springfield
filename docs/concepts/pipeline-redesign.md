...
**Effort**: M. Prompt updates for Ralph to handle the OHECI pattern for assumption breaks.

**What TODO-{id}.md gains**:
An explicitly separated Constraint Layer containing:
- Proposed architectural hypotheses
- Relevant ADRs to uphold
- Targeted quality thresholds (Farley/Adzic)
- Identified tech-debt landmines to avoid

**Files to touch**:
- `.pi/prompts/lisa.prompt.md.tpl` — focus output on constraints and hypotheses
- `.pi/prompts/ralph.prompt.md.tpl` — empower autonomous architectural pivots
- `docs/adr/ADR-007` — update model to Constraint-Driven (remove `scope.toml`)
  ...
  ─────────────────────────────────────────────────────────
    ...
    [Lisa — fires once per epic]
      Loop: OLA (query td for Bart's retrospective signals, watermarked)
            ...
            → PAE (produce TODO-{id}.md with constraint/context layers)
      Produces: TODO-{id}.md (intent + context + constraints + hypotheses)
...
**Ralph's Context Under the Constraint-Driven Model:**
Rather than dictating files (`scope.toml`) or hallucinating bash scripts (`guardrails.sh`), the redesign relies on Ralph's capabilities as an Archi-Engineer. 
...
| **Total** | **~1500 tokens + exploration** | **~200 tokens + focused TDD exploration** |

The orchestrator enforces baseline checks natively. Ralph executes autonomously, free to pivot the architecture if an assumption breaks.
...
- Implement Priority 3 (Constraint-Driven Handoff)
- Lisa now produces robust constraint boundaries; Ralph operates as an Archi-Engineer.
- Ralph's context shrinks; execution becomes highly focused via TDD.
...
- Lisa's handoff — Constraint-driven context boundaries rather than task slicing.
- Ralph's role — Elevated to an Archi-Engineer who autonomously pivots on assumption breaks.
